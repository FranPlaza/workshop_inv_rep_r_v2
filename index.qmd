...
A lo largo de este workshop, vamos a utilizar herramientas como:

- GIT - GitHub: Para mantener el control de versiones de nuestra investigación

- LaTeX: Para la redacción de documentos científicos

- R y RStudio: Para hacer los análisis y como plataforma base

- Quarto: Para integrar análisis y generación de distintos documentos
...
- **Reproduciblidad Computacional**: Transparencia completa en lo que respecta al código, software, hardware y los detalles específicos de la implementación.

- **Reproduciblidad Empírica**: Configuraciones y protocolo del experimento realizado.

- **Reproducibilidad estadística**: Proporciona transparencia en la elección de pruebas estadísticas, los parámetros del modelo y los valores de umbral. Es fundamental para abordar problemas como el "p-hacking", donde se manipulan los datos o las pruebas hasta obtener un resultado "significativo".
...
![](images/paper_nature.png){scale=60% fig-align="center"}

["1.500 scientist lift the lid on reproducibility"](https://www.nature.com/articles/533452a) [@baker20161]

::: {.notes}
Un artículo en Nature del año 2016 sobre la reproducibilidad en la ciencia indica que más del 70% de los investigadores han fallado en reproducir experimentos de otros y más de la mitad en reproducir los suyos. A pesar de la percepción de una crisis de reproducibilidad, la mayoría aún confía en la literatura publicada. Los esfuerzos para mejorar la reproducibilidad varían entre disciplinas y a menudo enfrentan obstáculos como la reticencia de las revistas a publicar replicaciones fallidas. Sin embargo, hay un amplio apoyo para mejorar las prácticas, incluyendo un diseño experimental más robusto y mejor mentoría.
:::


## Difusión y citas

![](images/paper_open_science.png){scale=50% fig-align="center"}

["Point of View: How open science helps researchers succeed"](https://elifesciences.org/articles/16800) [@mckiernan2016open]


::: {.notes}
En el 2016 se efectuó un estudio ([McKiernan et al 2016](https://elifesciences.org/articles/16800)) en donde se muestra el *índice de citación relativa* que corresponde a la tasa de citaciones medias para artículos publicados en revistas científicas de  ciencia abierta versus las citaciones para revistas. Esto para 19 disciplinas de la ciencia.

Acá podemos ver que los artículos puiblicados como ciencia abierta tienen hasta casi 3 veces el número de citas que los que no son de ciencia abierta. En la parte superior del gráfico, para ciencias aplicadas y física.
...
**Y en este sentido, para ayudar a solucionar estas brechas, podemos utilizar software de código abierto**
:::
...
::: {.notes}
Vamos a hablar un poco respecto a herramientas para la investigación reproducible

**Re-usable**: Permite a los investigadores tener ventaja del trabajo adelantado de los demás. A muchos nos ha pasado que no podemos implementar algún código y buscamos en internet en sitios como stack-exchange o stack-overflow, GitHub, etc. El principio es "para qué inventar la rueda, si alguien ya logró inventarla", mejor construir sobre lo ya desarrollado y adaptarlo a nuestro contexto

**Comprobable**: Donde los proyectos pueden permitir a la comunidad el poder leer los códigos y hacer pruebas de su funcionamiento. De esta manera encontrar y corregir errores más rápido y de la misma forma, otros investigadores pueden validar sus resultados

**Colaborativo**: Además de los puntos anteriores, que ya lo hacen colaborativo. Permite trabajar en proyectos mucho más demandantes, que no podrían ser llevados a cabo por un sólo investigador. Hay ejemplos de desarrollos como por ejemplo las librerías de python como scipy o numpy, o el mismo tensorflow que es ampliamente utilizado en machine learning y deep learning.
:::
...
::: {.notes}
La imagen muestra la clasificacion de los distintos tipos de investigación científica en un rango que va desde "No reproducible" hasta el "Estándar de oro". En el extremo izquierdo del espectro se encuentra sólo la publicación, indicando un nivel bajo de reproducibilidad. A medida que avanzamos en el espectro, se agregan componentes adicionales que mejoran la reproducibilidad: "Código", "Código y datos", y luego "Código y datos vinculados y ejecutables". El "Estándar de oro" al final del espectro muestra la "Replicación completa", implicando la máxima reproducibilidad. Este espectro destaca la importancia de compartir no solo las publicaciones, sino también el código y los datos subyacentes para permitir la verificación y replicación completas de los resultados científicos.
:::
...
::: {.notes}
**Investigadores**
Los entornos de trabajo de los investigadores va cambiando en la medida que los software se van actualizando, si es que instalan nuevo software o  usan distintos computadores, por ejemplo. Si no se logra capturar el ambiente que contiene el proyecto y los investigadores necesitan volver a su proyecto después de meses o años (como es común en la investigación), no van a poder replicar los resultados o no podrán tener la suficiente confianza que los resultados son los mismos. No van a tener forma de saber qué cambios se han producido en un ambiente de investigación específico y qué impacto podrían tener esos cambios en su capacidad para ejecutar el código y en los resultados.

**Colaboradores**
Como estamos en un entorno de colaboración, y al trabajar en varios ambientes computacionales abre una posibilidad no menor de encontrar muchos  errores. Entonces, el tratar de solucionar este tipo de problemas puede ser ser un proceso bastante lento y frustrante, sobre todo si los investigadores tienen que averiguar cuáles son las diferencias entre los ambientes computacionales de cada uno y lo que puede causar los errores. En este sentido, algunos errores pueden no ser detectados y pueden afectar potencialmente los resultados.

**Pirámide**
La imagen muestra una pirámide que está dividida en cuatro niveles que representan diferentes componentes de un proyecto de investigación. La base de la pirámide, el "Ambiente", es la base sobre la que se construye todo el proyecto. Justo encima, hay dos bloques que forman el segundo nivel: "Código" y "Datos", que son esenciales para el proceso de investigación y están apoyados por el ambiente subyacente. El tercer nivel tiene que ver con la "Documentación", que abarca los dos bloques inferiores. Y finalmente, en la cima de la pirámide se encuentra "Artículo", lo que  p representa la meta final de la investigación, que es publicar un artículo científico.
:::
...
::: {.notes}
Hay varias maneras de capturar entornos computacionales. Acá mostramos algunos Sistemas de Gestión de Paquetes, Binder, Máquinas Virtuales y Contenedores. Cada uno tiene sus ventajas y desventajas, y la opción más apropiada va a depender de la naturaleza del  proyecto.

Podemos dividirlos en dos categorías: aquellos que capturan solo el software y sus versiones usados en un entorno (Sistemas de Gestión de Paquetes de ANACONDA), y aquellos que replican un entorno computacional completo, incluyendo el sistema operativo y configuraciones personalizadas (Máquinas Virtuales y Contenedores).

Otra forma de clasificarlos es por cómo se presenta la investigación reproducida al que la reproduce. Usar Binder o una Máquina Virtual crea un resultado mucho más gráfico, del tipo GUI. En cambio, los resultados de Contenedores y Sistemas de Gestión de Paquetes de ANACONDA se interactúan más fácilmente a través de la línea de comandos.
:::
...
::: {.notes}
BinderHub, usa tecnología en la nube que es la queda vida a Binder. 

La arquitectura de BinderHub a grosso modo. BinderHub junta todas las herramientas tecnológicas que trabajan en armonía para crear entornos de computación personalizados y listos para usar en la nube. Acá se puede configurar todo el ambiente necesario para que el proyecto funcione como corresponda. Se necesitan algunos recursos, claro, como buen espacio en la nube, un poco de conocimiento en tecnologías de contenedores y  configuración.
:::
...
Esta parte será muy importante porque Quarto utiliza mucho la sintáxis para escritura técnica de LaTeX
...
:::{.fragment}
[Artículo original sobre Tidy Data en Journal of Statistical Software](https://www.jstatsoft.org/article/view/v059i10) [@tidy]
:::
